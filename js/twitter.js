...
function relative_time(time_value) {
  ...
  if (delta < 60) {
    ...
    return 'Cerca de un minuto atras';
  } else if(delta < (60*60)) {
    return (parseInt(delta / 60)).toString() + ' minutos atras';
  } else if(delta < (120*60)) {
    return 'Cerca de una hora atras';
  } else if(delta < (24*60*60)) {
    return 'Cerca de ' + (parseInt(delta / 3600)).toString() + ' horas atras';
  } else if(delta < (48*60*60)) {
    return '1 dia atras';
  } else {
    return (parseInt(delta / 86400)).toString() + ' dias atras';
  }
}